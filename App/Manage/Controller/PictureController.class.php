 ...
 * @Last Modified time: 2017-12-04 09:54:12
 */
...
class PictureController extends CommonContentController {
	...
	public function index() {
		...
		if ($this->pid) {
			...
			$where = array('picture.delete_status' => 0, 'cid' => array('in', $idarr));
		} else {
			$where = array('picture.delete_status' => 0);
		}
		...
		$art   = D2('ArcView', 'picture')->nofield('content,picture_urls')->where($where)->order('picture.point DESC, picture.id DESC')->limit($limit)->select();
	...
	public function addPost() {
		...
		if ($id = M('picture')->add($data)) {
			...
			$this->success('添加成功', U('Picture/index', array('pid' => $this->pid)));
		} else {
			$this->error('添加失败');
		}
	}
	...
	public function editPost() {
		...
		if (false !== M('picture')->save($data)) {
			...
			$this->success('修改成功', U('Picture/index', array('pid' => $this->pid)));
		} else {
	...
	public function move() {
		...
		if (IS_POST) {
			...
			$model_id = D2('ArcView', 'picture')->where(array('id' => array('in', $id)))->getField('model_id');
			if (empty($model_id)) {
			...
			if (false !== M($actionName)->where(array('id' => array('in', $id)))->setField('cid', $cid)) {
				...
				$this->success('移动成功', U('Picture/index', array('pid' => $this->pid)));
			} else {
				$this->error('移动失败');
			}
			exit();
	...
	public function trach() {
		...
		$where = array('picture.delete_status' => 1);
		$count = D2('ArcView', 'picture')->where($where)->count();
		...
		$page->setConfig('theme', '%HEADER% %FIRST% %UP_PAGE% %LINK_PAGE% %DOWN_PAGE% %END%');
		$limit = $page->firstRow . ',' . $page->listRows;
		$art   = D2('ArcView', 'picture')->nofield('content,picture_urls')->where($where)->order('picture.id DESC')->limit($limit)->select();
	...
	public function del() {
		...
		$model_id = D2('ArcView', 'picture')->where(array('id' => $id))->getField('model_id');
		if (empty($model_id)) {
			$this->error('文档不存在，请刷新重试');
		}

		if (false !== M('picture')->where(array('id' => $id))->setField('delete_status', 1)) {
			//set delete_status =1
			...
			$this->success('删除成功', U('Picture/index', array('pid' => $this->pid)));
	...
	public function delBatch() {
		...
		$model_id = D2('ArcView', 'picture')->where(array('id' => array('in', $idArr)))->getField('model_id');
		if (empty($model_id)) {
			$this->error('文档不存在，请刷新重试');
		}

		if (false !== M('picture')->where(array('id' => array('in', $idArr)))->setField('delete_status', 1)) {
			...
			$this->success('批量删除成功', U('Picture/index', array('pid' => $this->pid)));
	...
	public function restore() {
		...
		if (false !== M('picture')->where(array('id' => $id))->setField('delete_status', 0)) {
			//set delete_status =0
			M('SearchAll')->where(array('arc_id' => $id, 'model_id' => $model_id))->setField('delete_status', 0);

			$this->success('还原成功', U('Picture/trach', array('pid' => $this->pid)));
	...
	public function restoreBatch() {
		...
		if (false !== M('picture')->where(array('id' => array('in', $idArr)))->setField('delete_status', 0)) {
			//set delete_status =0
	...
	public function clear() {
		...
		if (M('picture')->delete($id)) {
			...
			$this->success('彻底删除成功', U('Picture/trach', array('pid' => $this->pid)));
		} else {
			$this->error('彻底删除失败');
		}
	}
